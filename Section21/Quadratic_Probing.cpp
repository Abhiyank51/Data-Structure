...
#define SIZE 10   // define constant size 10
 ...
 // in linear probinng - H[(index+i)%SIZE] but in quadratric probins only change H[(index+i)%SIZE] - >H[(index+i*i)%SIZE]
...
int probe(int H[], int key){
    ...
    return (index+i*i)%SIZE; // this is the free space found 
}
